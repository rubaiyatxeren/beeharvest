...
function initGlassNav() {
  const navInner = document.getElementById('bhNavInner');
  const items    = navInner?.querySelectorAll('.bh-nav-item[data-page]');
  if (!navInner || !items) return;

  function updatePill(activeEl) {
    const navRect  = navInner.getBoundingClientRect();
    const itemRect = activeEl.getBoundingClientRect();
    const left     = itemRect.left - navRect.left - 6;
    navInner.style.setProperty('--pill-left', left + 'px');
    navInner.style.cssText = navInner.style.cssText;
    // Update via a dynamic style rule
    let s = document.getElementById('_bhPill');
    if (!s) {
      s = document.createElement('style');
      s.id = '_bhPill';
      document.head.appendChild(s);
    }
    s.textContent = `.bh-nav-inner::before{left:${left}px!important;width:${itemRect.width}px!important;}`;
  }
  ...
  document.getElementById('mobileCartBtn')
    ?.addEventListener('click', (e) => { e.preventDefault(); openMobileCart(); });

  // Init pill on first paint
  requestAnimationFrame(() => {
    const active = navInner.querySelector('.bh-nav-item.active');
    if (active) updatePill(active);
  });
}

// Also update active state when navigateTo is called
const _origNavigateTo = window.navigateTo;
window.navigateTo = function(page) {
  _origNavigateTo(page);
  document.querySelectorAll('.bh-nav-item[data-page]').forEach(i => {
    i.classList.toggle('active', i.dataset.page === page);
  });
  const active = document.querySelector('.bh-nav-item.active');
  if (active && document.getElementById('bhNavInner')) {
    const s = document.getElementById('_bhPill');
    if (s) {
      const navRect  = document.getElementById('bhNavInner').getBoundingClientRect();
